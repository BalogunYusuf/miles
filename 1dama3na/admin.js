...
function initials(f, l) {
  return ((f && f[0]) || "?").toUpperCase() + ((l && l[0]) || "");
}
...
function avatarHTML(m, sizeStyle) {
  ...
  if (m && m.profileImageUrl) {
    return `<div class="avatar" style="padding:0; overflow:hidden; ${extra}"><img src="${esc(
      m.profileImageUrl
    )}" alt="${esc(
      fullName(m)
    )}" style="width:100%; height:100%; object-fit:cover; display:block;"></div>`;
  }
  return `<div class="avatar" style="${avatarStyle(
    m && m.memberId
  )} ${extra}">${initials(m && m.firstName, m && m.lastName)}</div>`;
}
...
function sameId(a, b) {
  return Boolean(idOf(a) && idOf(a) === idOf(b));
}
...
function assignedAdminName(profile) {
  ...
  if (typeof profile.assignedAdmin === "object")
    return (
      profile.assignedAdmin.name || profile.assignedAdmin.email || "Assigned"
    );
  if (sameId(profile.assignedAdmin, state.admin?._id))
    return state.admin?.name || "Assigned to you";
...
const ICONS = {
  ...
  female:
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="9" r="6"/><line x1="12" y1="15" x2="12" y2="22"/><line x1="9" y1="19" x2="15" y2="19"/></svg>',
  clock:
  ...
  heart:
    '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 1 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8z"/></svg>',
  ...
  pause:
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>',
  trash:
...
function showView(name, el) {
  ...
  if (
    ...
    ["administrators", "settings"].includes(name)
  ) {
    name = "dashboard";
    el = null;
  }
  ...
  if (el) {
    ...
    if (el.classList.contains("nav-sub")) {
      let top = el
        .closest(".nav-group")
        .querySelector(".nav-item:not(.nav-sub)");
...
function onViewShown(name) {
  ...
  if (name === "notifications" && !state.notificationsLoaded)
    loadNotifications();
  if (name === "analytics") loadAnalytics();
  if (name === "reports") renderReports();
  if (name === "administrators" && state.admin?.role === "super_admin")
    loadAdministrators();
  if (
    name === "settings" &&
    state.admin?.role === "super_admin" &&
...
async function initHeader() {
  try {
    ...
    document.getElementById("chipAvatar").textContent = initials(
      parts[0],
      parts[1] || ""
    );
    document.getElementById("chipName").textContent = state.admin.name;
    document.getElementById("chipRole").textContent = roleLabel(
      state.admin.role
    );
    ...
    document.getElementById(
      "dashGreeting"
    ).textContent = `${greeting}, ${parts[0]}`;
    document.getElementById(
      "dashSubtitle"
    ).textContent = `Here's how the congregation of members is doing today, ${new Date().toLocaleDateString(
      "en-US",
...
function runGlobalSearch() {
  ...
  showView("members");
  document
    .querySelector('.nav-item[data-view="members"]')
...
async function loadDashboard() {
  ...
  try {
    ...
    const kpis = [
      { label: "Total Profiles", value: s.totalProfiles, icon: ICONS.users },
      {
        ...
        value: s.approvedProfiles,
        icon: ICONS.check,
      },
      {
        label: "Pending Profiles",
        value: s.pendingProfiles,
        icon: ICONS.clock,
      },
      {
        label: "Suspended Profiles",
        value: s.suspendedProfiles,
        icon: ICONS.x,
      },
    ...
    grid.innerHTML = `<div class="card kpi">Couldn't load stats: ${esc(
      err.message
    )}</div>`;
  }
  ...
  try {
    ...
    document.getElementById("activityFeed").innerHTML = logs.length
      ? logs
          .map(
            (l) =>
              `<div class="feed-item"><div class="feed-dot"></div><div class="feed-text">${esc(
                l.description
              )}<div class="t">${timeAgo(l.createdAt)}</div></div></div>`
          )
          .join("")
    ...
    document.getElementById(
      "activityFeed"
    ).innerHTML = `<div class="feed-item"><div class="feed-text">Activity log isn't available for your role.</div></div>`;
  }
  ...
  try {
    ...
    document.getElementById(
      "dashMiniTable"
    ).innerHTML = `<tbody><tr><td>Couldn't load recent profiles.</td></tr></tbody>`;
  }
}
function renderDashMini(profiles) {
  const rows = profiles
    ...
    <tr>
      ...
      <td>${esc(
        fullName(m)
      )}<div style="font-size:10.5px;color:var(--text-muted);">${esc(
        m.memberId
      )}</div></td>
      <td><span class="badge ${statusBadgeClass(m.status)}">${statusLabel(
        m.status
      )}</span></td>
  ...
  document.getElementById(
    "dashMiniTable"
  ).innerHTML = `<thead><tr><th></th><th>Member</th><th>Verification</th><th>Congregation</th></tr></thead>
     <tbody>${
       rows ||
       '<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:20px;">No profiles yet.</td></tr>'
     }</tbody>`;
}
...
function updateMembersViewHeading() {
  ...
  if (state.memberViewMode === "assigned-to-me") {
    ...
    subtitle.textContent =
      "Profiles assigned to you, including work waiting to be claimed or completed.";
  } else {
    title.textContent = "All Profiles";
    subtitle.textContent =
      "Manage member profiles, verification, and assignment workflow.";
  }
}
...
function memberSource() {
  return state.memberViewMode === "assigned-to-me"
...
async function loadMembers() {
  ...
  try {
    ...
    tbody.innerHTML = `<tr><td colspan="11" style="text-align:center; color:var(--text-muted); padding:30px;">Couldn't load members: ${esc(
      err.message
    )}</td></tr>`;
  }
}
...
async function loadAssignedProfiles() {
  ...
  try {
    const res = await AdminAPI.get(
      "/profiles/my-assigned?limit=100&sort=-assignedAt"
    );
    ...
    tbody.innerHTML = `<tr><td colspan="11" style="text-align:center; color:var(--text-muted); padding:30px;">Couldn't load assigned profiles: ${esc(
      err.message
    )}</td></tr>`;
  }
}
...
function populateFilterOptions(list) {
  const countryCodes = [
    ...new Set(list.map((m) => m.country).filter(Boolean)),
  ...
  fCountry.innerHTML =
    ...
    countryCodes
      .map(
        (c) =>
          `<option value="${esc(c)}" ${
            c === curCountry ? "selected" : ""
          }>${esc(countryName(c))}</option>`
      )
      .join("");
...
function populateAssigneeFilter() {
  ...
  select.innerHTML = `
    ...
    ${admins
      .map(
        (a) =>
          `<option value="${esc(a._id)}">${esc(a.name || a.email)}</option>`
      )
      .join("")}
  `;
...
function renderMembers() {
  ...
  let list = memberSource().filter((m) => {
    if (gender && (m.gender || "").toLowerCase() !== gender.toLowerCase())
      return false;
    if (statuses && !statuses.includes(m.status)) return false;
    if (
      assignmentStatus &&
      (m.assignmentStatus || "unassigned") !== assignmentStatus
    )
      return false;
    ...
    if (
      assignee &&
      assignee !== "unassigned" &&
      !sameId(m.assignedAdmin, assignee)
    )
      return false;
    if (
      country &&
      String(m.country || "").toUpperCase() !== String(country).toUpperCase()
    )
      return false;
    if (
      congregation &&
      !(m.congregation || "").toLowerCase().includes(congregation)
    )
      return false;
    if (minAge !== undefined && (m.age === undefined || m.age < minAge))
      return false;
    if (maxAge !== undefined && (m.age === undefined || m.age > maxAge))
      return false;
    if (bap === "Yes" && !m.baptismYear) return false;
    if (bap === "No" && m.baptismYear) return false;
    if (pio === "Yes" && (!m.pioneerStatus || m.pioneerStatus === "none"))
      return false;
    if (pio === "No" && m.pioneerStatus && m.pioneerStatus !== "none")
      return false;
    if (q) {
      const hay = `${fullName(m)} ${m.memberId || ""} ${m.email || ""} ${
        m.congregation || ""
      } ${countryName(m.country)}`.toLowerCase();
  ...
  list = list.slice().sort((a, b) => {
    ...
    return (
      new Date(b.createdAt || b.assignedAt || 0) -
      new Date(a.createdAt || a.assignedAt || 0)
    );
  });
  ...
  document.getElementById("membersTbody").innerHTML =
    ...
    <tr>
      ...
      <td><span class="badge ${assignmentBadgeClass(
        m.assignmentStatus
      )}">${assignmentStatusLabel(m.assignmentStatus)}</span></td>
      <td>
        <div class="assigned-to-cell">
          <strong>${esc(assignedAdminName(m))}</strong>
          ${
            m.claimedBy
              ? `<small>Claimed by ${esc(
                  typeof m.claimedBy === "object"
                    ? m.claimedBy.name || m.claimedBy.email || "admin"
                    : "admin"
                )}</small>`
              : ""
          }
      ...
      <td>
        <div class="row-actions">
          <button title="View" onclick="openProfile('${m._id}')">${
          ICONS.eye
        }</button>
          <button title="Suggest a match" onclick="openMatchModal('${
            m._id
          }')">${ICONS.link}</button>
          ${workflowActionButtons(m)}
          ${statusActionButtons(m)}
          <button title="Delete" class="danger" onclick="deleteMember('${
            m._id
          }')">${ICONS.trash}</button>
        </div>
      </td>
      ...
      .join("") ||
    `<tr><td colspan="11" style="text-align:center; color:var(--text-muted); padding:30px;">No members match these filters.</td></tr>`;
}

function rowActionsForStatus(m) {
  if (m.status === "approved")
    return `<span class="badge matched">Eligible</span>`;
  if (m.status === "pending")
    return `<span class="badge pending">Awaiting Review</span>`;
...
function workflowActionButtons(profile) {
  ...
  if (isSuperAdmin()) {
    actions += `<button title="${
      profile.assignedAdmin ? "Reassign" : "Assign"
    } profile" onclick="openAssignmentModal('${profile._id}')">${
      ICONS.edit
    }</button>`;
  }
...
async function refreshProfileCollections(profileId, modalTab) {
  ...
  if (
    profileId &&
    document.getElementById("profileModal").classList.contains("open")
  ) {
...
async function approveMember(id) {
  try {
    await AdminAPI.patch(`/profiles/${id}/approve`, {});
    await refreshProfileCollections(id);
    loadDashboard();
  } catch (err) {
    alert("Could not approve profile: " + err.message);
  }
}
async function rejectMember(id) {
  ...
  if (!reason) return;
  try {
    await AdminAPI.patch(`/profiles/${id}/reject`, { note: reason });
    await refreshProfileCollections(id);
    loadDashboard();
  } catch (err) {
    alert("Could not reject profile: " + err.message);
  }
}
async function suspendMember(id) {
  ...
  if (!reason) return;
  try {
    await AdminAPI.patch(`/profiles/${id}/suspend`, { note: reason });
    await refreshProfileCollections(id);
    loadDashboard();
  } catch (err) {
    alert("Could not suspend profile: " + err.message);
  }
}
async function reactivateMember(id) {
  try {
    await AdminAPI.patch(`/profiles/${id}/reactivate`, {});
    await refreshProfileCollections(id);
    loadDashboard();
  } catch (err) {
    alert("Could not reactivate profile: " + err.message);
  }
}
async function deleteMember(id) {
  if (
    !confirm(
      "Delete this profile? This can only be undone by an administrator directly in the database."
    )
  )
    return;
  try {
    await AdminAPI.delete(`/profiles/${id}`);
    closeModal();
...
async function openAssignmentModal(profileId) {
  ...
  const profile =
    memberSource().find((p) => p._id === profileId) ||
  ...
  document.getElementById("assignmentModalTitle").textContent =
    profile?.assignedAdmin ? "Reassign Profile" : "Assign Profile";
  document.getElementById("assignmentModalMeta").textContent = profile
    ? `${fullName(profile)} · ${countryName(profile.country)}`
    : "Choose an administrator";
  document.getElementById("assignmentAdminSelect").innerHTML =
    '<option value="">Loading administrators…</option>';
  ...
  try {
    ...
    select.innerHTML = eligible.length
      ? `<option value="">Select an administrator</option>${eligible
          .map(
            (a) => `
          <option value="${esc(a._id)}" ${
              sameId(profile?.assignedAdmin, a._id) ? "selected" : ""
            }>
    ...
    document.getElementById("assignmentAdminHint").textContent = eligible.length
      ? `Only active administrators covering ${countryName(
          country
        )} or international profiles are shown.`
      : `No active administrator currently covers ${countryName(
          country
        )}. Update administrator coverage first.`;
  } catch (err) {
    document.getElementById("assignmentAdminSelect").innerHTML =
      '<option value="">Could not load administrators</option>';
...
async function claimProfile(profileId) {
  if (!confirm("Claim this profile and take responsibility for its review?"))
    return;
  try {
...
async function renderSearchResults() {
  ...
  const countryInput = document.getElementById("sCountry").value.trim();

  const country =
    Object.entries(COUNTRY_NAMES).find(
      ([code, name]) =>
        code.toLowerCase() === countryInput.toLowerCase() ||
        name.toLowerCase() === countryInput.toLowerCase()
    )?.[0] || countryInput;
    ...
    grid.innerHTML =
      ...
      <div class="card result-card">
        <div class="result-top">
          ${avatarHTML(m)}
          <div>
            <div style="font-weight:600; font-size:13.5px;">${esc(
              fullName(m)
            )}</div>
            <div style="font-size:11.5px; color:var(--text-muted);">${
              m.age ?? "—"
            } · ${formatLocation(m)}</div>
          </div>
          <span class="badge ${statusBadgeClass(
            m.status
          )}" style="margin-left:auto;">${statusLabel(m.status)}</span>
        </div>
        <div class="bio">${esc(m.aboutMe || "No bio provided yet.")}</div>
        <div class="result-actions">
          <button class="btn btn-ghost btn-sm" onclick="openProfile('${
            m._id
          }')">View Profile</button>
          <button class="btn btn-primary btn-sm" onclick="openMatchModal('${
            m._id
          }')">Create Match</button>
        </div>
      </div>`
        ...
        .join("") ||
      `<div class="card result-card">No profiles match this search.</div>`;
  } catch (err) {
    grid.innerHTML = `<div class="card result-card">Search failed: ${esc(
      err.message
    )}</div>`;
  }
}
...
async function loadMatches() {
  ...
  try {
    ...
    board.innerHTML = `<div class="kanban-col"><div class="kanban-col-head"><h4>Couldn't load matches</h4></div><div style="padding:12px; font-size:12.5px; color:var(--text-muted);">${esc(
      err.message
    )}</div></div>`;
  }
}
...
function renderMatchFilterBar() {
  ...
  bar.innerHTML = `<div class="card" style="padding:10px 16px; margin-bottom:14px; display:flex; align-items:center; gap:10px; font-size:12.5px; color:var(--text-muted);">
    Showing: <strong style="color:var(--text-main);">${esc(
      MATCH_FILTER_LABELS[state.matchFilterMode] || state.matchFilterMode
    )}</strong>
...
function renderKanban() {
  ...
  const board = document.getElementById("kanbanBoard");
  const allowedStages = state.matchFilterMode
    ? MATCH_FILTER_GROUPS[state.matchFilterMode]
    : null;
  const visibleStages = allowedStages
    ? STAGES.filter((s) => allowedStages.includes(s.key))
    : STAGES;
  board.innerHTML =
    visibleStages
      .map((stage) => {
    ...
    <div class="kanban-col">
      <div class="kanban-col-head"><h4>${
        stage.label
      }</h4><span class="kanban-count">${cards.length}</span></div>
      <div class="kanban-dropzone" data-stage="${
        stage.key
        ...
        ${cards
          .map((m) => {
            const A = m.profileA || {},
              B = m.profileB || {};
            return `
          <div class="card match-card" draggable="true" ondragstart="onDragStart(event,'${
            m._id
          }')" onclick="openMatchTimeline('${m._id}')">
            <div class="match-pair">
              ${avatarHTML(A, "width:36px; height:36px;")}
              ${avatarHTML(B, "width:36px; height:36px;")}
            </div>
            <div class="match-names">${esc(A.firstName || "?")} ${esc(
              (A.lastName || "?")[0] || ""
            )}. &amp; ${esc(B.firstName || "?")} ${esc(
              (B.lastName || "?")[0] || ""
            )}.</div>
            <div class="match-meta"><span>${fmtDate(m.createdAt)} · ${esc(
              m.createdBy?.name || "—"
            )}</span><span class="cscore">${
              m.compatibilityScore ?? "—"
            }%</span></div>
            <select class="stage-select" title="Move to a different stage" onclick="event.stopPropagation()" onchange="event.stopPropagation(); moveMatchStage('${
              m._id
            }', this.value)">
              ${STAGES.map(
                (s) =>
                  `<option value="${s.key}" ${
                    s.key === m.status ? "selected" : ""
                  }>${s.label}</option>`
              ).join("")}
            </select>
          </div>`;
      ...
      .join("") ||
    `<div class="kanban-col"><div class="kanban-col-head"><h4>No matches in this view</h4></div></div>`;
}
...
async function openMatchTimeline(id) {
  ...
  document.getElementById(
    "matchTimelineTitle"
  ).textContent = `Match History Timeline — ${fullName(
    match.profileA || {}
  )} & ${fullName(match.profileB || {})}`;
  ...
  try {
    ...
    document.getElementById("matchTimeline").innerHTML = events.length
      ? events
        ...
        <div class="timeline-item">
          <div style="display:flex; flex-direction:column; align-items:center;"><div class="tdot"></div>${
            i < events.length - 1 ? '<div class="tline"></div>' : ""
          }</div>
          <div class="timeline-content"><div class="tt">${esc(
            ev.title
          )}</div><div class="tn">${esc(ev.note)}</div><div class="tm">${
              ev.when
            } · ${esc(ev.by)}</div></div>
        </div>`
          )
    ...
    document.getElementById(
      "matchTimeline"
    ).innerHTML = `Couldn't load match history: ${esc(err.message)}`;
  }
}
...
function matchWizardSearchProfiles(q) {
  ...
  matchWizardSearchTimer = setTimeout(async () => {
    ...
    try {
      const res = await AdminAPI.get(
        `/profiles?status=approved&q=${encodeURIComponent(q)}&limit=10`
      );
      const list = res.data.profiles || [];
      box.innerHTML =
        list
          .map(
            (m) => `
        <div class="card" style="padding:10px 14px; margin-bottom:8px; display:flex; align-items:center; gap:10px; cursor:pointer;" onclick="selectMatchProfileA('${
          m._id
        }')">
          ${avatarHTML(m)}
          <div><div style="font-weight:600; font-size:13px;">${esc(
            fullName(m)
          )}</div><div style="font-size:11.5px; color:var(--text-muted);">${esc(
              m.memberId
            )} · ${esc(m.congregation || "")}</div></div>
        </div>`
          )
          .join("") ||
        '<div style="color:var(--text-muted); font-size:13px;">No approved profiles found.</div>';
    } catch (err) {
...
function renderSuggestionGroup(title, list, sourceProfile, international) {
  ...
  return `
    <div class="suggestion-group">
      <div class="section-title">${esc(title)} <span class="kanban-count">${
    list.length
  }</span></div>
      ${list
        .map((b) => renderSuggestionCard(b, sourceProfile, international))
        .join("")}
    </div>`;
}
...
async function selectMatchProfileA(profileId) {
  ...
  try {
    ...
    const sameCountry = suggestions.filter(
      (b) =>
        String(b.country || "").toUpperCase() ===
        String(A.country || "").toUpperCase()
    );
    const international = suggestions.filter(
      (b) =>
        String(b.country || "").toUpperCase() !==
        String(A.country || "").toUpperCase()
    );
    ...
    body.innerHTML = `
      <div class="card" style="padding:12px 14px; display:flex; align-items:center; gap:10px; margin-bottom:16px;">
        ${avatarHTML(A)}
        <div><div style="font-weight:600;">${esc(
          fullName(A)
        )}</div><div style="font-size:11.5px; color:var(--text-muted);">${esc(
      A.memberId
    )} · ${esc(A.congregation || "")} · ${esc(
      ...
      <div id="matchWizardSuggestions">
        ${
          suggestions.length
            ? renderSuggestionGroup("Same Country", sameCountry, A, false) +
              renderSuggestionGroup("International", international, A, true)
...
async function openProfile(id, preferredTab) {
  ...
  try {
    ...
    state.activeModalTab = modalTabs.includes(preferredTab)
      ? preferredTab
      : "Personal";
    ...
    if (m.profileImageUrl) {
      ...
      avatar.innerHTML = `<img src="${esc(m.profileImageUrl)}" alt="${esc(
        fullName(m)
    ...
    document.getElementById("mHeadMeta").innerHTML = `
      ${esc(m.memberId)} ·
      <span class="badge ${statusBadgeClass(m.status)}">${statusLabel(
      m.status
    )}</span> ·
      <span class="badge ${assignmentBadgeClass(
        m.assignmentStatus
      )}">${assignmentStatusLabel(m.assignmentStatus)}</span> ·
  ...
  if (isSuperAdmin()) {
    buttons.push(
      `<button class="btn btn-primary" onclick="openAssignmentModal('${
        m._id
      }')">${m.assignedAdmin ? "Reassign Profile" : "Assign Profile"}</button>`
    );
  }
  if (status === "assigned" && (assignedToCurrent || isSuperAdmin())) {
    buttons.push(
      `<button class="btn btn-primary" onclick="claimProfile('${m._id}')">Claim Profile</button>`
    );
  }
  if (status === "claimed" && (claimedByCurrent || isSuperAdmin())) {
    buttons.push(
      `<button class="btn btn-primary" onclick="completeProfileAssignment('${m._id}')">Complete Assignment</button>`
    );
  }
...
function renderAssignmentTab(m) {
  ...
  const coverage = assigned
    ? assigned.canHandleInternational
      ...
      : (assigned.assignedCountries || []).map(countryName).join(", ") ||
  ...
  return `
    <div class="assignment-summary">
      ...
      <div class="mp-grid">
        <div class="mp-field"><div class="k">Assigned Administrator</div><div class="v">${esc(
          assigned?.name || assigned?.email || "Unassigned"
        )}</div></div>
        <div class="mp-field"><div class="k">Assigned By</div><div class="v">${esc(
          assignedBy?.name || assignedBy?.email || "—"
        )}</div></div>
        <div class="mp-field"><div class="k">Assigned Date</div><div class="v">${fmtDateTime(
          m.assignedAt
        )}</div></div>
        <div class="mp-field"><div class="k">Country Coverage</div><div class="v">${esc(
          coverage
        )}</div></div>
        <div class="mp-field"><div class="k">Claimed By</div><div class="v">${esc(
          claimedBy?.name || claimedBy?.email || "—"
        )}</div></div>
        <div class="mp-field"><div class="k">Claimed Date</div><div class="v">${fmtDateTime(
          m.claimedAt
        )}</div></div>
      </div>
...
function renderModalTabs() {
  document.getElementById("mTabs").innerHTML = modalTabs
    .map(
      (t) =>
        `<div class="modal-tab ${
          t === state.activeModalTab ? "active" : ""
        }" onclick="state.activeModalTab='${t}'; renderModalTabs();">${t}</div>`
    )
    .join("");
  ...
  if (tab === "Personal") {
    body = `<div class="mp-grid">
      <div class="mp-field"><div class="k">Full Name</div><div class="v">${esc(
        fullName(m)
      )}</div></div>
      <div class="mp-field"><div class="k">Gender</div><div class="v">${esc(
        statusLabel(m.gender)
      )}</div></div>
      <div class="mp-field"><div class="k">Age</div><div class="v">${
        m.age ?? "—"
      }</div></div>
      <div class="mp-field"><div class="k">Location</div><div class="v">${formatLocation(
        m
      )}</div></div>
      <div class="mp-field"><div class="k">Congregation</div><div class="v">${esc(
        m.congregation || "—"
      )}</div></div>
      <div class="mp-field"><div class="k">Date Joined</div><div class="v">${fmtDate(
        m.createdAt
      )}</div></div>
      <div class="mp-field"><div class="k">Email</div><div class="v">${esc(
        m.email || "—"
      )}</div></div>
      <div class="mp-field"><div class="k">Phone</div><div class="v">${esc(
        m.phone || "—"
      )}</div></div>
    </div>`;
  } else if (tab === "Spiritual") {
    body = `<div class="mp-grid">
      <div class="mp-field"><div class="k">Baptism Year</div><div class="v">${
        m.baptismYear || "—"
      }</div></div>
      <div class="mp-field"><div class="k">Pioneer Status</div><div class="v">${esc(
        statusLabel(m.pioneerStatus)
      )}</div></div>
      <div class="mp-field"><div class="k">Congregation</div><div class="v">${esc(
        m.congregation || "—"
      )}</div></div>
      <div class="mp-field"><div class="k">Hours / Month</div><div class="v">${
        m.hoursPerMonth ?? "—"
      }</div></div>
    </div>
    ${
      m.spiritualGoals
        ? `<p class="bio" style="margin-top:14px;">${esc(m.spiritualGoals)}</p>`
        : ""
    }`;
  } else if (tab === "Lifestyle") {
    body = `<p class="bio" style="margin-bottom:14px;">${esc(
      m.aboutMe || "No bio provided yet."
    )}</p>
      <div class="mp-grid" style="margin-bottom:14px;">
        <div class="mp-field"><div class="k">Occupation</div><div class="v">${esc(
          m.occupation || "—"
        )}</div></div>
        <div class="mp-field"><div class="k">Education</div><div class="v">${esc(
          m.education || "—"
        )}</div></div>
      </div>
      ${
        (m.qualities || [])
          .map((q) => `<div class="pill-tag">${esc(q)}</div>`)
          .join("") || ""
      }`;
  } else if (tab === "Preferences") {
    body = `<div class="mp-grid">
      <div class="mp-field"><div class="k">Looking For</div><div class="v">${esc(
        m.lookingFor || "—"
      )}</div></div>
      <div class="mp-field"><div class="k">Relocation</div><div class="v">${esc(
        statusLabel(m.relocation)
      )}</div></div>
      <div class="mp-field"><div class="k">Children</div><div class="v">${esc(
        statusLabel(m.hasChildren)
      )}</div></div>
      <div class="mp-field"><div class="k">Preferred Contact</div><div class="v">${esc(
        statusLabel(m.preferredContact)
      )}</div></div>
    </div>`;
    ...
    body = history.length
      ? history
          ...
          <div style="display:flex; justify-content:space-between; font-size:11.5px; color:var(--text-muted); margin-bottom:8px;"><span>${esc(
            statusLabel(h.status)
          )}</span><span>${fmtDate(h.changedAt)}</span></div>
  ...
  try {
    ...
    document.getElementById("mpTimeline").innerHTML = matches.length
      ? matches
          .map((mt) => {
            const other =
              mt.profileA?._id === profileId ? mt.profileB : mt.profileA;
            ...
            <div class="timeline-content">
              ...
              <div class="tn">${esc(statusLabel(mt.status))} · ${
              mt.compatibilityScore ?? "—"
            }% compatible</div>
...
async function loadContacts() {
  ...
  try {
    ...
    if (state.contacts.length) openContact(state.contacts[0]._id);
      ...
      panel.innerHTML = `<div style="padding:24px; color:var(--text-muted);">No contact submissions yet.</div>`;
  } catch (err) {
    list.innerHTML = `<div style="padding:20px; color:var(--text-muted);">Couldn't load messages: ${esc(
      err.message
    )}</div>`;
  }
}
function renderInboxList() {
  ...
  list.innerHTML =
    ...
    <div class="inbox-item ${
      c._id === state.activeContactId ? "active" : ""
    }" onclick="openContact('${c._id}')">
      <div class="inbox-item-top">
        <span>${esc(c.name)}${
          c.status === "new"
            ? ' <span class="badge pending" style="margin-left:6px; vertical-align:middle;">New</span>'
            : ""
        }</span>
        <span class="date">${timeAgo(c.createdAt)}</span>
      </div>
      <div class="subj">${esc(c.subject || "(no subject)")}</div>
      <div class="subj" style="opacity:.75;">${esc(
        (c.message || "").slice(0, 70)
      )}${(c.message || "").length > 70 ? "…" : ""}</div>
    </div>`
      )
      .join("") ||
    `<div style="padding:20px; color:var(--text-muted);">No messages.</div>`;
}
async function openContact(id) {
  ...
  panel.innerHTML = `
    ...
    <div class="preview-meta">From <strong>${esc(c.name)}</strong> &lt;${esc(
    c.email
  )}&gt;${c.phone ? " · " + esc(c.phone) : ""} · ${fmtDateTime(
    c.createdAt
  )}</div>
      ...
      <div style="margin-bottom:20px;">
        ${(c.replies || [])
          ...
          <div style="background:var(--glass); border:1px solid var(--border-gold); border-radius:12px; padding:12px 16px; margin-bottom:10px;">
            <div class="t" style="color:var(--text-muted); font-size:10.5px; margin-bottom:5px;">${esc(
              r.repliedBy?.name || "Admin"
            )} · ${fmtDateTime(r.repliedAt)}</div>
            <div style="font-size:13px; line-height:1.6;">${esc(
              r.message
            )}</div>
          </div>`
          )
    ...
    <div class="preview-actions">
      <select class="filter-select" id="contactStatusSelect" onchange="updateContactStatus('${
        c._id
      }', this.value)">
        <option value="new" ${c.status === "new" ? "selected" : ""}>New</option>
        <option value="in_progress" ${
          c.status === "in_progress" ? "selected" : ""
        }>In Progress</option>
        <option value="resolved" ${
          c.status === "resolved" ? "selected" : ""
        }>Resolved</option>
      </select>
      <button class="btn btn-primary" onclick="sendContactReply('${
        c._id
      }')" style="margin-left:auto;">Send Reply</button>
    </div>`;
}
...
async function loadNotifications() {
  ...
  try {
    ...
    wrap.innerHTML = `<div class="card" style="padding:20px; color:var(--text-muted);">Couldn't load notifications: ${esc(
      err.message
    )}</div>`;
  }
}
...
async function loadAdministrators() {
  ...
  try {
    ...
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:var(--text-muted); padding:30px;">Couldn't load administrators: ${esc(
      err.message
    )}</td></tr>`;
  }
}
  ...
  tbody.innerHTML =
    admins
      .map((a) => {
        ...
        return `
      <tr>
        <td><div class="avatar" style="${avatarStyle(a._id)}">${initials(
          (a.name || "").split(" ")[0],
          (a.name || "").split(" ")[1]
        )}</div></td>
        <td>
        ...
        <td><span class="badge ${
          a.canHandleInternational || a.role === "super_admin"
        ...
        <td><span class="badge ${
          a.isActive === false ? "suspended" : "verified"
        }">${a.isActive === false ? "Inactive" : "Active"}</span></td>
        <td>
  <div class="row-actions">
    ${
      a.role === "super_admin"
        ? ""
        : `
      <button title="Edit country coverage" onclick="openCoverageModal('${
        a._id
      }')">
        ${ICONS.edit}
      </button>
    ...
    `
      ...
      .join("") ||
    `<tr><td colspan="8" style="text-align:center; color:var(--text-muted); padding:30px;">No administrators found.</td></tr>`;
}
...
function countryOptionsHTML(selected = []) {
  const selectedSet = new Set(
    (selected || []).map((c) => String(c).toUpperCase())
  );
  return Object.entries(COUNTRY_NAMES)
    ...
    .map(
      ([code, name]) =>
        `<option value="${code}" ${
          selectedSet.has(code) ? "selected" : ""
        }>${esc(name)} (${code})</option>`
    )
    .join("");
}
...
function openInviteAdminModal() {
  ...
  document.getElementById("adminModalSubtitle").textContent =
    "Set their role and country coverage before sending the invitation.";
  document.getElementById("adminIdentityFields").style.display = "";
  ["adminName", "adminEmail", "adminRole"].forEach(
...
async function openCoverageModal(adminId) {
  try {
    ...
    document.getElementById("adminInternational").checked =
      admin.canHandleInternational === true;
    document.getElementById("adminCountries").innerHTML = countryOptionsHTML(
...
function selectedAdminCountries() {
  return [...document.getElementById("adminCountries").selectedOptions].map(
    (option) => option.value
  );
}
...
async function submitAdminForm(event) {
  ...
  const canHandleInternational =
    document.getElementById("adminInternational").checked;
  const assignedCountries = canHandleInternational
    ? []
    : selectedAdminCountries();

  if (!canHandleInternational && assignedCountries.length === 0) {
    error.textContent =
      "Select at least one country or enable international access.";
    return false;
  }
  ...
  submit.textContent =
    state.activeAdminModalMode === "invite" ? "Sending…" : "Saving…";
...
async function loadSettings() {
  document.getElementById(
    "settingsNav"
  ).innerHTML = `<div style="padding:14px; color:var(--text-muted);">Loading…</div>`;
  ...
  try {
    ...
    document.getElementById(
      "settingsPanel"
    ).innerHTML = `<div style="padding:20px; color:var(--text-muted);">Couldn't load settings: ${esc(
      err.message
    )}</div>`;
  }
}
function renderSettingsNav() {
  document.getElementById("settingsNav").innerHTML = SETTINGS_TABS.map(
    (t) => `
    <div class="settings-nav-item ${
      t === state.activeSettingsTab ? "active" : ""
    }"
  ...
  if (tab === "General") {
    body = `
      <div class="sf-fields" style="margin-bottom:20px;">
        <div class="field"><label>Platform Name</label><input class="filter-input" data-path="platformName" value="${esc(
          s.platformName
        )}"></div>
        <div class="field"><label>Support Email</label><input class="filter-input" data-path="supportEmail" value="${esc(
          s.supportEmail
        )}"></div>
      </div>
      <div class="toggle-row">
        ...
        <div class="switch ${
          s.maintenanceMode.enabled ? "on" : ""
        }" data-path="maintenanceMode.enabled" onclick="toggleSwitchEl(this)"></div>
      </div>
      <div class="field" style="margin-top:14px;"><label>Maintenance Message</label><input class="filter-input" style="width:100%;" data-path="maintenanceMode.message" value="${esc(
        s.maintenanceMode.message
      )}"></div>`;
    ...
    body = [
      [
        "emailOnNewProfile",
        "Email on New Profile",
        "Notify admins when a new profile is submitted.",
      ],
      [
        "emailOnNewContact",
        "Email on New Contact",
        "Notify admins when a contact form is submitted.",
      ],
      [
        "emailOnNewMatch",
        "Email on New Match",
        "Notify admins when a match is created.",
      ],
      [
        "dailyAdminSummary",
        "Daily Admin Summary",
        "Send a daily digest of platform activity.",
      ],
      ...
      <div class="toggle-row">
        ...
        <div class="switch ${
          np[key] ? "on" : ""
        }" data-path="notificationPreferences.${key}" onclick="toggleSwitchEl(this)"></div>
      </div>`
      )
      .join("");
    ...
    body = `
      <div class="sf-fields" style="margin-bottom:20px;">
        <div class="field"><label>Minimum Age</label><input type="number" class="filter-input" data-path="matchRules.minAge" value="${
          mr.minAge
        }"></div>
        <div class="field"><label>Max Age Gap (years)</label><input type="number" class="filter-input" data-path="matchRules.maxAgeGapYears" value="${
          mr.maxAgeGapYears
        }"></div>
        <div class="field"><label>Auto-Archive After (days inactive)</label><input type="number" class="filter-input" data-path="matchRules.autoArchiveAfterDaysInactive" value="${
          mr.autoArchiveAfterDaysInactive
        }"></div>
      </div>
      <div class="toggle-row">
        ...
        <div class="switch ${
          mr.requireSameCountryByDefault ? "on" : ""
        }" data-path="matchRules.requireSameCountryByDefault" onclick="toggleSwitchEl(this)"></div>
      </div>`;
    ...
    body = [
      ...
      <div class="toggle-row">
        ...
        <div class="switch ${
          p[key] ? "on" : ""
        }" data-path="permissions.${key}" onclick="toggleSwitchEl(this)"></div>
      </div>`
      )
      .join("");
    ...
    body = `
      <div class="sf-fields" style="margin-bottom:20px;">
        <div class="field"><label>Session Timeout (minutes)</label><input type="number" class="filter-input" data-path="security.sessionTimeoutMinutes" value="${
          sec.sessionTimeoutMinutes
        }"></div>
        <div class="field"><label>Max Login Attempts</label><input type="number" class="filter-input" data-path="security.maxLoginAttempts" value="${
          sec.maxLoginAttempts
        }"></div>
        <div class="field"><label>Lockout Duration (minutes)</label><input type="number" class="filter-input" data-path="security.lockoutMinutes" value="${
          sec.lockoutMinutes
        }"></div>
        <div class="field"><label>Password Minimum Length</label><input type="number" class="filter-input" data-path="security.passwordMinLength" value="${
          sec.passwordMinLength
        }"></div>
      </div>
      <div class="toggle-row">
        <div><div class="lbl">Require 2FA for Super Admin</div></div>
        <div class="switch ${
          sec.require2FAForSuperAdmin ? "on" : ""
        }" data-path="security.require2FAForSuperAdmin" onclick="toggleSwitchEl(this)"></div>
      </div>`;
  } else if (tab === "Backup") {
    const b = s.backup;
    body = `
      <div class="toggle-row">
        ...
        <div class="switch ${
          b.autoBackupEnabled ? "on" : ""
        }" data-path="backup.autoBackupEnabled" onclick="toggleSwitchEl(this)"></div>
      </div>
      <div class="sf-fields" style="margin-top:16px;">
        <div class="field"><label>Frequency</label>
          <select class="filter-select" data-path="backup.frequency">
            <option value="daily" ${
              b.frequency === "daily" ? "selected" : ""
            }>Daily</option>
            <option value="weekly" ${
              b.frequency === "weekly" ? "selected" : ""
            }>Weekly</option>
            <option value="monthly" ${
              b.frequency === "monthly" ? "selected" : ""
            }>Monthly</option>
          </select>
        </div>
        <div class="field"><label>Retention (days)</label><input type="number" class="filter-input" data-path="backup.retentionDays" value="${
          b.retentionDays
        }"></div>
      </div>`;
  }
...
function analyticsChartOptions({ legend = false, scales = true, cutout } = {}) {
  ...
  if (scales) {
    options.scales = {
      x: {
        grid: { display: false },
        ticks: { color: t.muted, font: { family: "Inter", size: 10 } },
        border: { color: t.grid },
      },
      y: {
        beginAtZero: true,
...
function renderRankedAnalyticsList(elementId, rows, nameKey) {
  ...
  el.innerHTML = `<div class="analytics-list">${clean
    .map(
      (row) => `
    <div class="analytics-list-row">
      <div class="analytics-list-name" title="${esc(
        nameKey === "country" ? countryName(row[nameKey]) : row[nameKey]
      )}">${esc(
        nameKey === "country" ? countryName(row[nameKey]) : row[nameKey]
      )}</div>
      <div class="analytics-list-count">${Number(row.count) || 0}</div>
      <div class="analytics-list-track"><div class="analytics-list-fill" style="width:${Math.max(
        4,
        ((Number(row.count) || 0) / max) * 100
      )}%"></div></div>
    </div>`
    )
    .join("")}</div>`;
...
function renderAnalytics() {
  ...
  state.analyticsCharts.monthly = new Chart(
    document.getElementById("monthlyRegistrationsChart"),
    {
      type: "line",
      data: {
  ...
  const male = Number(
    gender.male ?? gender.Male ?? gender.brother ?? gender.Brother ?? 0
  );
  const female = Number(
    gender.female ?? gender.Female ?? gender.sister ?? gender.Sister ?? 0
  );
  state.analyticsCharts.gender = new Chart(
    document.getElementById("genderRatioChart"),
    {
      ...
      data: {
        ...
        datasets: [
          {
            data: [male, female],
            backgroundColor: [t.gold, t.goldLight],
            borderColor: [t.panel, t.panel],
            borderWidth: 3,
            hoverOffset: 5,
          },
        ],
  ...
  state.analyticsCharts.age = new Chart(
    document.getElementById("ageDistributionChart"),
    {
      ...
      data: {
        ...
        datasets: [
          {
            label: "Profiles",
            data: ages.map((x) => Number(x.count) || 0),
  ...
  state.analyticsCharts.approval = new Chart(
    document.getElementById("approvalRateChart"),
    {
      ...
      data: {
        ...
        datasets: [
          {
            data: decided ? [approved, rejected] : [1, 0],
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text:
              approval.approvalRate == null
                ? "No decisions yet"
                : `${approval.approvalRate}%`,
            color: t.goldLight,
            position: "top",
            font: { family: "DM Sans", size: 24, weight: "700" },
            padding: { bottom: 6 },
          },
        },
      },
  ...
  renderRankedAnalyticsList(
    "congList",
    a.mostActiveCongregations,
    "congregation"
  );
  ...
  const kpis = [
    {
      label: "Pending Profiles",
      value: a.pendingProfiles ?? a.dashboard?.pendingProfiles ?? 0,
    },
    {
      label: "New Contact Messages",
      value: a.pendingContacts ?? a.dashboard?.newContacts ?? 0,
    },
    {
      label: "Successful Matches",
      value: a.successfulMatches ?? a.dashboard?.marriedMatches ?? 0,
    },
  ];
  document.getElementById("analyticsExtraKpis").innerHTML = kpis
    .map(
      (k) =>
        `<div class="card kpi"><div class="kpi-label">${esc(
          k.label
        )}</div><div class="kpi-value">${Number(k.value) || 0}</div></div>`
    )
    .join("");
  renderReports();
}
...
async function loadAnalytics() {
  ...
  try {
    ...
    const message = `<div class="analytics-empty">Could not load analytics.<br>${esc(
      err.message
    )}</div>`;
...
const REPORT_TYPES = [
  {
    ...
    description:
      "Complete member profile register and current approval statuses.",
  },
  {
    type: "matches",
    title: "Matches Report",
    description:
      "Match records, compatibility scores, and relationship progress.",
  },
  {
    type: "contacts",
    title: "Contact Submissions",
    description: "Messages received through the public contact form.",
  },
  {
    type: "activityLogs",
    title: "Activity Logs",
    description: "Recent administrator actions and system activity.",
  },
  {
    type: "analytics",
    title: "Analytics Summary",
    description: "High-level profile, contact, match, and approval metrics.",
  },
];

function reportCardsHTML() {
  return REPORT_TYPES.map(
    (report) => `
    <div class="card report-card">
      <div class="rt-icon">${ICONS.file || ICONS.users}</div>
      <div><h4>${esc(report.title)}</h4><p>${esc(report.description)}</p></div>
      <div class="export-row">
        <button class="btn btn-ghost" onclick="downloadReport('${
          report.type
        }','pdf',this)">PDF</button>
        <button class="btn btn-ghost" onclick="downloadReport('${
          report.type
        }','excel',this)">Excel</button>
        <button class="btn btn-ghost" onclick="downloadReport('${
          report.type
        }','csv',this)">CSV</button>
      </div>
